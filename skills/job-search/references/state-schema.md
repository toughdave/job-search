...
Optional application `milestones` contain immutable `{at, source_ids}` records for `discovered_at`, `qualified_at`, `materials_ready_at` and `submitted_at`; the pipeline helper validates them and prevents rewriting. Optional nonempty `discovery_source` and `application_channel` labels stay separate. See [measurement](measurement.md). Career-round details live in versioned application notes with source IDs, tentative/confirmed status and packet references; preserve the existing stage/outcome enum values.

Optional `onboarding.resume.provenance_ledger` is `{file, sha256, source_id}` matching a registered `document` source. Use [provenance operations](provenance.md) to save the next version and pointer together. Source records retain the original draft hash, resume-source inventory and previous source ID for safe retries. Old stale pointers remain readable, but `plan.provenance.ready` is false until repaired; new ledger registrations cannot leave an older pointer behind. No schema reset or deletion of candidate history is needed.

Optional `applications[].form_review` is `{file, sha256, source_id}` for an immutable registered document source. Use [form completion](form-completeness.md) and `form_review.py save`, not an invented pass flag. New/changed `ready` records require a current passing review; legacy ready entries stay readable but need rechecking before reuse. `materials_ready_at` describes document readiness separately. After evidence or application changes, use `form_review.py commit-update` to save the full intended state draft and move existing ready applications with stale reviews to `preparing` atomically, then save fresh inspected reviews. The helper returns the affected IDs and preserves previous review sources. No workspace schema reset is required.
