...
At the first useful handoff after setup/interview, write a short private `notes/how-to-return.md` under the verified records workspace. Include the current app/provider, actual project and records locations, the chosen summon action, native discovery status (observed or unverified), and the one current next action. Keep account credentials out. Update it on an app/provider change; reread the project pointer and pending question before resuming. Do not invent a new state-schema field or mark pending interview topics complete to create the note.
