...
Use this route only for a message the person asked you to draft for someone else. Do not move candidate interview questions into the block to evade pacing. Multiple outgoing messages can be separate saved files; include at most one in each checked reply and link the others. Inspect the meaning and recipient yourself: the checker cannot establish whom a sentence addresses or whether the candidate requested it. A short label such as `Draft for the recruiter, not sent:` must be the last nonblank line immediately above the block. This makes the intended audience visible; it does not prove the recipient or prevent a misleading candidate question inside the draft. Inspect that meaning yourself. Return `send_verbatim` unchanged. No email, message or calendar action is authorized by this check.
...
`plan` returns `pending_application_questions`, `application_answers` and `application_action_reminders`. The last list identifies application next actions still holding the temporary `Reconcile candidate answer ...` message. After acting on an answer, commit a concrete next step, such as `Candidate reviews the saved clarification draft before sending`. `finish-reply` refuses when the answer source used for this turn still has that reminder. It does not block independent work because an unrelated application has one. `save-answer` accepts the returned question ID and the entire response using the existing arguments. Its saved answer retains `application_id`; map that source into application facts/logistics without treating it as profile coverage. An answer is not automatically consent to send a message or accept an event. On restart, reuse the saved question or answer. Application replies may summarize delivered files in a short list, but must end with exactly the saved question and contain no other requests.
...
Before repeating a saved question, verify its logistics against the original invitation. `ask` and `check-reply` refuse explicit English weekday/date mismatches and impossible dates. They return a nonblocking review note when the year is missing. Do not assume a year or repeat an error with a disclaimer above it. Uppercase `SAT` followed by a date and an immediate numeric score is treated as the exam, including `(score 1400)` and `– scored 1400`; a bare `SAT` weekday still receives the calendar check. Impossible dates remain errors.
...
This saves a missing message as an immutable statement before checking the reply. If step 3 already saved the full message with `save-answer` or `record-statement`, supply `--capture-source-id SOURCE_ID`; the helper verifies exact equality and avoids a duplicate. A partial source is refused. Keep `save-answer` for actual pending questions; this final capture does not invent an interview answer or map facts automatically. If the latest captured answer or statement matches but you omit the source ID, the helper refuses with the saved source ID instead of creating a duplicate. It does not assume two identical messages belong to one turn. For a genuinely new turn with identical text, use `record-statement` with that new turn ID first, then finish using the same ID or its returned source. Different turns need different message IDs even if their text is identical. For a repeated short reply such as “Thanks” or “Continue,” record the genuinely new turn before finishing instead of waiting for the duplicate guard to refuse it. Reuse a source only for the same received turn; text equality alone is not proof. A failed reply check may leave the valid message captured; retry with the same turn ID and fresh revision.
...
When the person stops or asks how to return, as well as at the first useful setup handoff, follow the main skill's return instructions and [details](harnesses.md#first-setup-handoff). Save the private return note and a concrete next action before replying. Give the person only their current app's summon action, actual project and records locations and one next action. If topics remain deferred, fact mapping is unfinished or a tool is blocked, describe that actual partial status rather than claiming all setup is finished. Keep this guidance in the private project so a restart or app change does not restart the interview. Do not repeat it after every answer or start a new question when the person is leaving.
