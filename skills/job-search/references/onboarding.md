...
`check-reply` now returns `send_verbatim` and a send-only instruction, without a success-status label. Output that value alone. A bold, italic or quoted final question is allowed when its wording still exactly matches the pending question. A heading such as “What I saved:” is declarative; indirect requests such as “You can also share your phone number” and validation prefaces are refused. A recognized relocation/stay-local either/or question is one preference decision. These remain bounded English heuristics, not a general language parser or host composer interceptor.
