...
**OpenCode / T3:** discover only the current agent/provider's exposed account tools. Do not assume the Codex or Claude desktop connection carries over. Use `--harness other` and fresh current-host account/read evidence as described in [harness capabilities](harnesses.md#capability-limits-in-other-hosts); if access is absent, offer pasted job replies and keep mail pending.
