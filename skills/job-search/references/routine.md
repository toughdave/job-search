...
Every scheduled run uses the same [saved-form completion checks](form-completeness.md) as a manual application. Required fields alone are insufficient: compare independently expected work history and education with the saved portal, inspect relevant supported optional fields and correct parser omissions. Retain the validated review and recheck after edits or expired sessions. An unfinished or failed check leaves the application preparing/blocked, with the exact next action; never recommend submission from document readiness alone. Submission authorization is separate. Existing schedules already load this reference at every run; regenerate a saved prompt through the routine helper when updating its text without changing the user's timing, scope or consent.
