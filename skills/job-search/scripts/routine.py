...
def prompt(root, project):
    ...
    return {'prompt': (
        ...
        'For mail, follow references/mail.md: verify the live authenticated account against integrations.mail for this harness before inbox reads. A saved connection or another host is not proof of current access. Use mail_accounts.py plan with the freshly returned email; if not ready, keep mail blocked and continue independent discovery. Never report unavailable mail as no replies. '
        ...
        'Use references/discovery.md for adaptive title/duty queries, screening labels and search hygiene. For employer responses use references/career-stages.md to prepare the relevant next-stage pack without sending or creating events absent authorization. Follow references/writing.md and references/forms.md for cover-letter completion and clean upload copies.\n'
        ...
        'Before calling an application ready or recommending/performing submission, follow references/form-completeness.md. Compare independently expected work history and education with the saved form, check required and supported recommended/optional fields, persist live review evidence, and run form_review.py save and check. Missing entries, unsupported omissions or stale reviews block readiness. Recheck after changes or session recovery; final submission authorization remains separate.\n'
